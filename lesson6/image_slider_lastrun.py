...
expName = 'image_slider'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\kurokida\\OneDrive - Kyushu University\\学内ワークショップ\\prog-stat-ws-github\\lesson6\\image_slider_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    slider = visual.Slider(win=win, name='slider',
        ...
        labels=("非常にxxでない","非常にxxである"), ticks=(1, 2, 3, 4, 5), granularity=0.0,
        style='rating', styleTweaks=(), opacity=None,
        labelColor='LightGray', markerColor='Red', lineColor='White', colorSpace='rgb',
        font='MS PGothic', labelHeight=0.05,
        flip=False, ori=0.0, depth=-3, readOnly=False)
    mouse = event.Mouse(win=win)
    x, y = [None, None]
    mouse.mouseClock = core.Clock()
    next_button = visual.ImageStim(
        win=win,
        name='next_button', 
        image='next_button.png', mask=None, anchor='center',
        ori=0.0, pos=(0.4, -0.45), draggable=False, size=(0.2, 0.2),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('image_file_list.xlsx'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[fixation, key_resp, stim, slider, mouse, next_button],
        )
            ...
            if mouse.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames(next_button, namespace=locals())
